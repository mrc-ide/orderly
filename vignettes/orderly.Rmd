...
In this vignette we will expand on these two aims, and show that the first one is a prerequisite for the second. The second is more interesting though and we start there. If you just want to get started using `orderly`, you might prefer `vignette("introduction")`, which is a walk-through of major `orderly` features.  If you are already familiar with version 1 you might prefer `vignette("migrating")`, which highlights differences between the versions and describes the migration paths available.

# Collaborative analysis

Many analyses only involve a single person and a single machine; in this case there are any number of workflow tools that will make orchestrating this analysis easy.  In a workflow model you have a [**graph**](https://en.wikipedia.org/wiki/Directed_acyclic_graph) of dependencies over your analysis, over which data flows. So for example you might have 
...
A separate issue we have faced with this perspective on graph-based workflows is the idea that there **is** a final state that we are trying to get the graph to.  If the above workflow is being performed in the context of a regularly updated data set, and if the reports at the end have been used or sent to collaborators in the past, then we have many "final" versions.  In this perspective, we do not want to replace versions as we run them, but to have a system where we can store and address versions after they have been run, keeping track of what went into them.  This is the same information that goes into the workflow-orchestrator pattern, but exposed differently.
...
To read more about `orderly`'s approach to collaborative analysis, see

* `vignette("dependencies")`, which describes the process of creating links in a dependency graph
* `vignette("collaboration")`, which describes the process of distributing artefacts and metadata about them among people working on parts of the graph.

But first, you probably need to understand `orderly`'s approach to reproducibility.
...
Start with `vignette("introduction")`, which walks through the basic features of the package.
