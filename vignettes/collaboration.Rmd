...
When pulling packets, first the user pulls metadata from the server. When `orderly::orderly_location_metadata_pull` is run, all metadata from the server is updated on the client. We now know everything the server holds, and the hashes of all the files contained in each packet.
...
It is important that the `.outpack` directory is *not* shared via git; we warn about this now, and you can use `orderly::ordery_gitignore_update()` to automatically create a suitable `.gitignore` file that will prevent it being accidentally committed.  See `vignette("troubleshooting")` for more information on this.
...
Without running `orderly::orderly_init()`, they will get an error prompting them to initialise the repo (see the initial example).
