---
title: "Introduction to orderly"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to orderly}
...
You might also prefer the [orderly tutorial](https://mrc-ide.github.io/orderly-tutorial/) which works through similar material in slide form, or [watch a short talk](https://www.youtube.com/watch?v=lkPgihFQbrk) that describes the ideas in the package, and why it differs to other approaches to reproducibility and workflows.
...
To run the report and create a new **packet**, use `orderly::orderly_run()`:

```{r, orderly_root = path}
id <- orderly::orderly_run("incoming_data")
id
```
...
dest <- tempfile()
fs::dir_create(dest)
orderly::orderly_copy_files(id, files = c("final.rds" = "data.rds"),
                            dest = dest)
```

which copies `data.rds` to some new temporary directory `dest` with name `final.rds`. This uses `orderly`'s `outpack_` functions, which are designed to interact with outpack archives regardless of how they were created (`orderly` is a program that creates `outpack` archives). Typically these are lower-level than `orderly_` functions.
...
writeLines(c(
  'orderly::orderly_dependency("incoming_data", "latest()",',
...
Here, we've used `orderly::orderly_dependency()` to pull in the file `data.rds` from the most recent version (`latest()`) of the `data` packet with the filename `incoming.rds`, then we've used that file as normal to make a plot, which we've saved as `analysis.png`.

We can run this just as before, using `orderly::orderly_run()`:

```{r, orderly_root = path}
id <- orderly::orderly_run("analysis")
```
...
The function `orderly::orderly_dependency()` is designed to operate while the packet runs. These functions all act by adding metadata to the final packet, and perhaps by copying files into the directory.

* `orderly::orderly_description()`: Provide a longer name and description for your report; this can be reflected in tooling that uses orderly metadata to be much more informative than your short name.
* `orderly::orderly_parameters()`: Declares parameters that can be passed in to control the behaviour of the report, returning a list of the parameters. Parameters are key-value pairs of simple data (booleans, numbers, strings) which your report can respond to. They can also be used in queries to `orderly::orderly_dependency()` to find packets that satisfy some criteria.
* `orderly::orderly_resource()`: Declares that a file is a *resource*; a file that is an input to the the report, and which comes from this source directory. By default, orderly treats all files in the directory as a resource, but it can be useful to mark these explicitly, and necessary to do so in "strict mode" (see below). Files that have been marked as a resource are **immutable** and may not be deleted or modified.
* `orderly::orderly_shared_resource()`: Copies a file from the "shared resources" directory `shared/`, which can be data files or source code located at the root of the orderly repository. This can be a reasonable way of sharing data or commonly used code among several reports.
* `orderly::orderly_artefact()`: Declares that a file (or set of files) will be created by this report, before it is even run.  Doing this makes it easier to check that the report behaves as expected and can allow reasoning about what a related set of reports will do without running them. By declaring something as an artefact (especially in conjunction with "strict mode") it is also easier to clean up `src` directories that have been used in interactive development (see below).
* `orderly::orderly_dependency()`: Copy files from one packet into this packet as it runs, as seen above.
* `orderly::orderly_strict_mode()`: Declares that this report will be run in "strict mode" (see below).
...
Here, we've added a block of special orderly commands; these could go anywhere, for example above the files that they refer to. If strict mode is enabled (see below) then `orderly::orderly_resource` calls must go before the files are used as they will only be made available at that point (see below).
...
writeLines(c(
  "pars <- orderly::orderly_parameters(n_samples = 10)",
...
```r
pars <- orderly::orderly_parameters(n_samples = NULL)
```

to define a parameter with no default, or defined multiple parameters with

```r
pars <- orderly::orderly_parameters(n_samples = 10, distribution = "normal")
```
...
```{r, orderly_root = path}
id <- orderly::orderly_run("random", list(n_samples = 15))
```

Our resulting file has 15 rows, as the parameter we passed in affected the report:

```{r, orderly_root = path}
orderly::orderly_copy_files(id, files = c("random.rds" = "data.rds"),
                            dest = dest)
...
```{r, orderly_root = path}
orderly::orderly_search('latest(name == "random")')
```

But we can also pass in parameter queries here:

```{r, orderly_root = path}
orderly::orderly_search('latest(name == "random" && parameter:n_samples > 10)')
```

These can be used within `orderly::orderly_dependency()` (the `name == "random"` part is implied by the first `name` argument), for example

```r
orderly::orderly_dependency("random", "latest(parameter:n_samples > 10)",
...
```r
orderly::orderly_dependency("random", "latest(parameter:n_samples == this:n)",
...
writeLines(c(
  'orderly::orderly_shared_resource("data.csv")',
  'orderly::orderly_artefact(description = "analysis", "analysis.png")',
  "",
  'd <- read.csv("data.csv")',
  'png("analysis.png")',
  "plot(y ~ x, d)",
...
```{r, orderly_root = path}
id <- orderly::orderly_run("use_shared")
```
...
* only copy files from the source directory (`src/<reportname>/`) to the draft directory where the report runs (`draft/<reportname>/<packet-id>`) that were declared with `orderly::orderly_resource`; this leaves behind any extra files left over in development
...
For example, suppose that we have interactively run our `incoming_data/incoming_data.R` script, we would leave behind generated files.  We can report on this with `orderly::orderly_cleanup_status`:
...
```{r, orderly_root = path}
orderly::orderly_cleanup_status("incoming_data")
```
...
```{r, orderly_root = path}
orderly::orderly_cleanup("incoming_data")
```

There is a `dry_run = TRUE` argument you can pass if you want to see what would be deleted without using the status function.

You can also keep these files out of git by using the `orderly::orderly_gitignore_update` function:

```{r, orderly_root = path}
orderly::orderly_gitignore_update("incoming_data")
```
...
If you delete packets from your `archive/` directory then this puts `orderly` into an inconsistent state with its metadata store. Sometimes this does not matter (e.g., if you delete old copies that would never be candidates for inclusion with `orderly::orderly_dependency` you will never notice).  However, if you delete the most recent copy of a packet and then try and depend on it, you will get an error.
...
id_latest <- orderly::orderly_search("latest", name = "incoming_data")
...
orderly::orderly_run("analysis")
```

The error here tries to be fairly informative, telling us that we failed because when copying files from `r inline(id_latest)` we found that the packet was corrupt, because the file `data.rds` was not found in the archive.  It also suggests a fix; we can tell `orderly` that `r inline(id_latest)` is "orphaned" and should not be considered for inclusion when we look for dependencies.

We can carry out the suggestion and just validate this packet by running

```{r echo = FALSE, results = "asis"}
r_output(
  sprintf('orderly::orderly_validate_archive("%s", action = "orphan")',
          id_latest))
```
...
```{r, orderly_root = path}
orderly::orderly_validate_archive(action = "orphan")
```
...
If you want to remove references to the orphaned packets, you can use `orderly::orderly_prune_orphans()` to remove them entirely:
...
orderly::orderly_config_set(core.use_file_store = TRUE)
dir_tree(path, ".outpack", all = TRUE)
```
...
The `orderly` package is built on a metadata and file storage system called `outpack`; we will be implementing support for working with these metadata archives in other languages (see [`outpack_server`](https://github.com/mrc-ide/outpack_server) for our server implementation in Rust and [`outpack-py`](https://github.com/mrc-ide/pyorderly) in Python).  The metadata is discussed in more detail in `vignette("metadata")` and we will document the general ideas more fully at [`mrc-ide/outpack`](https://github.com/mrc-ide/outpack)
